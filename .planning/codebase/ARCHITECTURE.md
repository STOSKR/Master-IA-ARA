...
1. phase0 probe command invokes CsfloatProbeService.
1. Probe service fetches JSON payload and stores timestamped dump.
1. phase0 catalog command parses latest dump into canonical catalog records.
1. Catalog repository writes deterministic JSON/CSV artifacts.

## Core data flow (extraction framework path)

1. AsyncExtractionKernel receives connectors and extraction targets.
1. Connector-target jobs are scheduled concurrently with semaphore limits.
1. Each connector runs probe-first fetch, parse, and cleaning pipeline.
1. Retry loop wraps each job and writes anomaly dump on exhaustion.
1. Kernel returns run metrics plus per-job success/failure envelopes.
